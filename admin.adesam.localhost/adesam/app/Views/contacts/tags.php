						...
						<tbody>
							...
							<?php else: ?>
								...
								<?php foreach ($datas as $key => $data): ?>
									<?php $dataContacts = $data->contacts; ?>
									<?php $index++; ?>
									...
									<tr>
										...
										<td class="text-wrap">
											<div class="d-flex flex-wrap column-gap-3 row-gap-2">
												<?php foreach ($dataContacts as $dataContact): ?>
													<div class="d-flex">
														<div class="flex-shrink-0">
															<img width="27px" height="27px" class="object-fit-cover"
																src="<?= $dataContact->fileSrc ?? '/assets/images/avatar-contact.png'; ?>"
																alt="<?= $dataContact->fileName ?? 'Image not set avatar used'; ?>">
														</div>
														<div class="flex-grow-1 ms-1">
															<a href="<?= $dataContact->contactCipherId; ?>"
																class="mb-0"><?= $dataContact->nickname; ?></a>
														</div>
													</div>
												<?php endforeach; ?>
											</div>
...
<!-- new_tag_modal -->
<?= view_cell('\App\Cells\UiCell::tagModal', ['action' => '/contacts/tags/create']); ?>
